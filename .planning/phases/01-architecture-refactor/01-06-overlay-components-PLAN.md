...
must_haves:
  truths:
    - "MiniPlayer — IIFE-компонент с mount(root)/render()/destroy() — подписан на ['currentTrack','isPlaying']"
    - "FullscreenPlayer — IIFE-компонент с mount(root)/render()/destroy() — подписан на ['currentTrack','isPlaying','duration','fullscreenOpen']"
    - "QueuePanel — IIFE-скелет с mount(root)/render()/destroy() (полная функциональность — Phase 6, тут только заготовка с пустым render)"
...
const MiniPlayer = (() => {
  ...
  function template(state) {
    ...
    return `
      ...
      <div id="mp-play-btn" data-action="toggle">${playing ? '⏸' : '▶'}</div>
    `;
  }

  function render() {
    const state = { currentTrack: Store.get('currentTrack'), isPlaying: Store.get('isPlaying') };
    root.style.display = state.currentTrack ? 'flex' : 'none';
    root.innerHTML = template(state);
    hydrate();
  }

  function hydrate() {
    // Use single delegated click on root: tap on play btn → toggle, tap elsewhere → expand FP
    root.onclick = (e) => {
  ...
  function mount(el) {
    root = el;
    unsub = Store.subscribe(['currentTrack', 'isPlaying'], render);
    render();
...
const FullscreenPlayer = (() => {
    ...
    return `
      ...
      <div class="fp-progress-wrap">
        ...
        <div class="fp-time">
          ...
          <span id="fp-time-total">${Utils.fmtTime(state.duration)}</span>
        </div>
      </div>
      <div class="fp-controls">
        <div data-action="prev">⏮</div>
        <div id="fp-play-btn" data-action="toggle">${playing ? '⏸' : '▶'}</div>
  ...
  function render() {
    const state = {
      currentTrack: Store.get('currentTrack'),
  ...
  function hydrate() {
    root.onclick = (e) => {
      ...
      if (!action) return;
      switch (action) {
        case 'close':         Store.set({ fullscreenOpen: false }); break;
    ...
    if (range) range.oninput = () => PlaybackEngine.seekPct(parseFloat(range.value));
  }

  function tickProgress() {
    if (!Store.get('fullscreenOpen')) {
      // not visible → stop loop
      cancelAnimationFrame(rafId);
      rafId = null;
      return;
    }
    ...
    if (prog && dur) prog.value = (pos / dur) * 100;
  ...
  function mount(el) {
    root = el;
    unsub = Store.subscribe(['currentTrack', 'isPlaying', 'duration'], render);
    Store.subscribe(['fullscreenOpen'], syncOpen);
    render();
    syncOpen();
  }
...
3. **Удалить** временные subscribe-блоки из Plan 04 (которые сейчас живут где-то в конце скрипта):
   ...
   - Также удалить функции `openFullPlayer()`/`closeFullPlayer()` — они больше не нужны (FullscreenPlayer hydrate() handler `close` сам пишет в Store, mini-player click-handler сам открывает). НО: `openFullPlayer` — это inline onclick на mini-player markup'е (строка 439). После того как MiniPlayer.mount() заменит innerHTML root'а, inline onclick исчезнет — поэтому удаление `openFullPlayer` безопасно. Аналогично `closeFullPlayer` исчезнет когда FullscreenPlayer перерендерит свой root.
   - **НО осторожно:** `openFullPlayer` на mini-player'е сработает один раз ДО того, как MiniPlayer.mount перепишет innerHTML. Чтобы избежать ReferenceError, оставить как stubs:
   ```js
...
Поместить эти строки **после** того, как загружены DOM (скрипт в конце body — DOM уже готов) и **после** определения `Router`.
  ...
  <acceptance_criteria>
    ...
    - **Удалены временные подписки Plan 04:** `grep -cE "Store\.subscribe\(\['currentTrack'\]" index.html` → `1` (только внутри MiniPlayer; FullscreenPlayer подписан на `['currentTrack', 'isPlaying', 'duration']` — отдельная подпись)
    - `grep -cE "function tickFpProgress" index.html` → `0`
    - Browser smoke: открыть страницу, включить трек → mini-player виден с обложкой/заголовком/артистом → тап на mini-player → fullscreen открывается с правильным треком → seek-bar движется → seek работает → ⏭/⏮ работают → 👎/👍 отправляют POST /wave/feedback → 🤍 (пустое сердце) → POST /library/like → ▾ закрывает FP → переключить таб с играющим треком → mini-player по-прежнему виден и реагирует на ⏸/⏵.
  </acceptance_criteria>
  <done>MiniPlayer и FullscreenPlayer — IIFE-компоненты с mount/render/destroy. Их рендер заменил временные подписки из Plan 04. Полноэкранный плеер реагирует на `fullscreenOpen` через подписку.</done>
</task>
